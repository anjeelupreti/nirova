...
import { formatDateTime, formatTime, withGregorian } from "@/lib/dates";
        ...
        <div className="text-right">
          ...
          {issued ? (
            <p className="mt-0.5 type-caption">
              {/* Both calendars on anything printed: the hospital files in
                  Bikram Sambat and an insurer or a ministry reads Gregorian,
                  and a sheet carrying one of them makes somebody convert by
                  hand. */}
              {withGregorian(issued)}{" "}
              {formatTime(issued)}
            </p>
          ) : null}
        </div>
